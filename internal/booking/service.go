...
// Service depends on the BookingStore interface, not a concrete store, so
// main.go can wire in RedisStore for production while tests wire in
// MemoryStore or ConcurentStore.
type Service struct {
	store BookingStore
}
